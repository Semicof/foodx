    ...
    <div className="mt-5">
      ...
      {comments
        ? comments.map((item, index) => (
            <div
              key={index}
              className="w-full mt-6 border rounded-lg overflow-hidden "
            >
              {/* Top */}
              <div className="flex gap-3 p-4 bg-red-400 text-white items-center justify-between">
                <div>
                  ...
                  />
                  <div className="">
                    <h2 className="text-xl font-bold">{item.user_name}</h2>
                    <span>{item.user_point} points</span>
                  </div>
                </div>
              ...
              {/* Center */}
              <div className="p-4">
                <h3 className="text-xl font-bold text-primary mt-4 mb-4">{item.review_title}</h3>
                <hr />

                <div className="flex items-center flex-wrap gap-4 m-2">
                  {item.images
                    ? item.images.map((img, index) => (
                      <Image src={img} width={100} height={100} alt="img" className="rounded-md"/>
                    ))
                    : "No image for this review"}
                </div>
                <hr />

                <p className="p-2">{item.review_content}</p>
